/**
 * @file test_imap_performance.cpp
 * @brief IMAP协议解析性能测试
 * 
 * 本测试文件专注于评估IMAP协议解析器的性能指标，特别是在高负载条件下的处理能力。
 * 主要功能：
 * 1. 性能测试 - 测量解析大量IMAP命令的吞吐量和延迟
 * 2. 压力测试 - 在极限条件下验证系统稳定性
 * 3. 内存使用测试 - 监控内存消耗情况
 * 4. 多线程并发测试 - 使用互斥锁保护共享资源
 * 
 * 测试方法：
 * - 使用随机生成的IMAP命令创建大量数据包
 * - 记录处理这些数据包所需的时间
 * - 计算每秒处理的命令数量
 * - 测试不同线程数和负载条件下的性能变化
 * 
 * 该测试与test_imap_parsing.cpp相似，但更专注于性能指标的量化测量，
 * 用于性能优化和系统调优，确保在实际部署中能够处理预期的流量负载。
 */
